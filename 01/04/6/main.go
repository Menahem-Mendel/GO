// измените программу lissajous так, 
// чтобы она генерировала изображение разных цветов, 
...
func lissajous(out io.Writer) {
	...
	for i := 0; i < nframes; i++ {
		...
		for t := 0.0; t < cycles*2*math.Pi; t += res {
			...
			img.SetColorIndex(size+int(x*size+0.5), size+int(y*size+0.5), uint8(i%paletteSize+1))
		}
		phase += 0.1
